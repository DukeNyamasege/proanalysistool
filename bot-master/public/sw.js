...
const SW_BASE_PATH = new URL(self.registration.scope).pathname.replace(/\/$/, '');
const withBasePath = path => `${SW_BASE_PATH}${path}`;
...
async function handleNavigation(request) {
    try {
        ...
        const indexResponse = (await caches.match(withBasePath('/'))) || (await caches.match(withBasePath('/index.html')));
        if (indexResponse) {
...
async function handleOfflineFallback(request) {
    ...
    if (request.headers.get('accept')?.includes('text/html')) {
        ...
        const cachedIndex = (await caches.match(withBasePath('/'))) || (await caches.match(withBasePath('/index.html')));
        if (cachedIndex) {
            return cachedIndex;
        }
...
function isAuthRequest(url) {
    ...
    return (
        ...
        // Matchestool auth/data endpoints (using secure domain validation)
        isAllowedDomain(url.hostname, 'matchestool.pro') ||
