...
class PWAManager {
    ...
    async registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
        ...
        try {
            const is_trading33 = /^\/Trading33(\/|$)/i.test(window.location.pathname);
            const service_worker_url = is_trading33 ? '/Trading33/sw.js' : '/sw.js';
            const service_worker_scope = is_trading33 ? '/Trading33/' : '/';
